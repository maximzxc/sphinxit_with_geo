...
class LazySelectTree(ConfigMixin):
    ...
    def __init__(self, indexes, geo=False, lat=None, lon=None, distance=None):
    ...
    def copy(self):
        new_tree = LazySelectTree(self._indexes, self.geo, self.lat, self.lon, self.distance).with_config(self.config)
    ...
    def SelectFrom(self):
        if self._nodes['SelectFrom'] is None:
            select_container = SelectFromContainer(indexes=self._indexes,
                                                   geo=self.geo,
    ...
    def Where(self):
        if self._nodes['Where'] is None:
            filters_container = FiltersContainer(self.geo, self.distance).with_config(self.config)
    ...
    def get_select_nodes(self):
        if self._nodes['SelectFrom'] is None:
            select_container = SelectFromContainer(indexes=self._indexes, geo=self.geo, lat=self.lat, lon=self.lon).with_config(self.config)
            self._nodes['SelectFrom'] = select_container
...
class Search(ConfigMixin):
    ...
    def __init__(self, indexes, config, geo=False,
                 connector=None, lat=None, lon=None, distance=None):
        super(Search, self).__init__()
        self._nodes = LazySelectTree(lat=lat, lon=lon, distance=distance, geo=geo, indexes=indexes).with_config(config)
