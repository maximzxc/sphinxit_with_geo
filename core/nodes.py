...
class SelectFromContainer(ConfigMixin):
    ...
    def __init__(self, geo=False, indexes=None, lat=None, lon=None):
        ...
        self.or_fields = []
        if self.geo:
            geodist = '*, SQRT(69.1*69.1*(latitude - %f)*(latitude - %f) + 53*53*(longitude - %f)*(longitude - %f)) as distance' % (self.lat, self.lat, self.lon, self.lon)
            self.fields.append(geodist)
...
class FiltersContainer(ConfigMixin):
    ...
    def __init__(self, geo=False, distance=None):
        super(FiltersContainer, self).__init__()
        self.query = []
        self.conditions = []
        self.geo = geo
        self.distance = distance
        cond = 'distance < %d' % (distance)
        if geo:
